...
class RawlsGCN():
    ...
    def fit(self,adj, feats, labels, idx_train, idx_val,idx_test, enable_cude=True, device_number=0, model='rawlsgcn_graph',
            split_seed=0, num_epoch=100, lr=0.05, weight_decay=5e-4, hidden=64, dropout=0.5, loss='negative_log_likelihood' ):
        ...
        configs = {
            ...
            "loss": self.loss,
            "seed": random_seed,
            "save_path": f"ckpts/{self.dataset}/{self.model}/lr={self.lr}_nepochs={self.num_epoch}_decay={self.weight_decay}_seed={random_seed}.pt",
        }

        trainer=self.run_exp(dataset, split, configs)
